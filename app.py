...
def get_program(
    ...
    # -----------------------------------------------------
    # Worker kimlik doğrulaması
    # -----------------------------------------------------
    worker_date = normalize_date(
        data.get("date") or data.get("targetDate") or data.get("programDate") or ""
    ) if (data.get("date") or data.get("targetDate") or data.get("programDate")) else ""
    worker_city = normalize_text(
        data.get("city") or data.get("hipodrom") or data.get("track") or data.get("venue") or ""
    )

    if worker_date and worker_date != iso_date:
        raise RuntimeError(
            f"Worker yanlış tarih döndürdü: istendi={iso_date}, gelen={worker_date}"
        )
    if worker_city and worker_city.casefold() != city.casefold():
        raise RuntimeError(
            f"Worker yanlış hipodrom döndürdü: istendi={city}, gelen={worker_city}"
        )
    ...
    result = {
        ...
        "debug": {
            ...
            "worker_response_date": worker_date,
            "worker_response_city": worker_city,
            "request_verified": bool(
                (not worker_date or worker_date == iso_date)
                and (not worker_city or worker_city.casefold() == city.casefold())
            ),
