...
def _history_before(rows, target_date):
    """Yalnızca hedef koşudan ÖNCEKİ gün ve daha eski yarışları döndürür.

    Kural: hedef tarih = T ise yalnızca tarih < T kullanılabilir.
    Aynı gün yapılan yarışlar (tarih == T) ve gelecekteki kayıtlar
    kesinlikle hesaba katılmaz.
    """
    td = _dt(target_date)
    if td is None:
...
def _family_values(prior,target,workouts=None,field=None):
    ...
    # Kritik kural: hedef koşunun tarihi T ise T günü ve sonrası geçmişten çıkarılır.
    # Böylece fonksiyon hangi çağrı noktasından kullanılırsa kullanılsın
    # aynı gün/future leakage oluşmaz.
    target_race_date = _first(target, ["date", "tarih", "Tarih"], None)
    prior = _history_before(prior, target_race_date)
    if not prior:return out
    ...
    if field:
        ...
        diffs=[]
        target_dt = _dt(_first(target, ["date", "tarih", "Tarih"], None))
        for other in field:
            if other is target:continue
            other_history = _history_before(_history(other), target_dt)
            for r in other_history:
                k=(str(_first(r,["date","tarih","Tarih"],"")),_city(r),_dist(r));p=_place(r)
...
def _current_values(horse,race,horses):
    target={"surface":race.get("surface") or (race.get("meta") or {}).get("surface",""),"distance":race.get("distance") or (race.get("meta") or {}).get("distance"),"condition":race.get("condition") or race.get("raceName") or (race.get("meta") or {}).get("detail",""),"date":race.get("date") or race.get("tarih") or race.get("target_date")}
    # Hesaplama girdisini açıkça hedef tarihten önceki geçmişle sınırla.
    prior = _history_before(_history(horse), target.get("date"))
    vals=_family_values(prior,target,_workouts(horse),horses)
